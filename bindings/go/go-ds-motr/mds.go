...
func main() {
    ...
    if err != nil {
        fmt.Println("error putting to datastore: ", err)
    }
    ...
    getval,err := motrds.Get(newk)

    fmt.Println("getval:", getval)
    if err != nil {
        fmt.Println("error getting value from datastore: ", err)
    }

    fmt.Println("getval:", string(getval))
    motrds.Mkv.Close()
